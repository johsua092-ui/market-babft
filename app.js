...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    // Cek apakah user adalah admin/moderator/owner
    ...
    if (existingSessions.length > 0) {
      ...
      if (existingDevice) {
        ...
        // Device baru - logout device lama
        console.log('Device baru terdeteksi, logout device lama');
...
async function verifyToken(req, res, next) {
  try {
    ...
    // Cek apakah session masih valid dan device sama
...
initDatabase().then(() => {
  app.listen(PORT, () => {
    ...
    console.log('1. POST   /api/auth/login      - Login');
    console.log('2. GET    /api/dashboard       - Dashboard (perlu token)');
    console.log('3. POST   /api/auth/logout     - Logout (perlu token)');
    console.log('4. GET    /api/devices         - Lihat device aktif (perlu token)');
    console.log('5. GET    /api/test            - Test server');
    console.log('');
  });
});
